 ...
 *
 */
public class RestResourceCallback implements CallBack<ResourceBean> {

	private static final Logger LOG = Logger.getLogger(RestResourceCallback.class);
	@Override
	public ResourceBean execute(Class<?> clazz) {
		...
		RestResource resource = clazz.getAnnotation(RestResource.class);
		resourceBean.setEnable(resource.enable());
		String name = resource.name();
		
		String parentPath = RestResource.RESOURCE_PATH + path.value();
