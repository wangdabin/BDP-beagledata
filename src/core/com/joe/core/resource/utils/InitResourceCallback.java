 ...
 *
 */
public class InitResourceCallback implements CallBack<ResourceBean> {

	private static final Logger LOG = Logger.getLogger(InitResourceCallback.class);
	@Override
	public ResourceBean execute(Class<?> clazz) {
		...
		InitResource resource = clazz.getAnnotation(InitResource.class);
		resourceBean.setEnable(resource.enable());
		String name = resource.name();
		
		String parentPath = InitResource.RESOURCE_PATH + path.value();
